...
def test_duplicate_slug_rejected(app):
    ...
    tenant = tenants.create_tenant(name="Dup", slug="dup-store", email="dup@test.com")
    products.create(tenant_id=tenant.id, name="One", slug="same-slug", price="1.00")
    with pytest.raises(ValidationError):
        products.create(tenant_id=tenant.id, name="Two", slug="same-slug", price="2.00")
...
def test_get_by_public_id_not_found(app):
    ...
    with pytest.raises(NotFoundError):
        products.get_by_public_id(tenant.id, "00000000-0000-0000-0000-000000000099")
